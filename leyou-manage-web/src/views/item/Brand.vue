<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
    <v-card>
        <v-card-title>
            ...
            <v-spacer/>
            <v-flex xs3>
                <v-text-field label="输入关键字搜索" @keydown.enter=""  v-model="search" append-icon="search" hide-details/>
            </v-flex>
        </v-card-title>
        <v-divider/>
        <v-data-table
    ...
    export default {
        ...
        methods: {
            ...
            deleteBrand (oldBrand) {
                ...
                this.isEdit = false;
                // 根据品牌信息查询商品分类
                this.$message.confirm ('确认修改数据吗?', '提示', {
                    confirmButtonText: '确定修改吗',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then (() => {
                    this.$http.delete ("/item/brand/delete/" + oldBrand.id, {params: {}})
                        .then (
